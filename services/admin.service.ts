...
export const adminService = {
  ...
  addProductVariant: async (id: string, data: any, config?: any) => api.post(`/admin/products/${id}/variants`, data, config),
  createProductAttributes: async (id: string, data: any) => api.post(`/admin/products/${id}/attributes`, data),
  bulkCreateVariants: async (id: string, data: any) => api.post(`/admin/products/${id}/variants/bulk`, data),
  bulkUpdateInventory: async (id: string, data: any) => api.post(`/admin/products/${id}/inventory/bulk`, data),
  updateProductVariant: async (productId: string, variantId: string, data: any, config?: any) => api.put(`/admin/products/${productId}/variants/${variantId}`, data, config),
  deleteProductVariant: async (productId: string, variantId: string) => api.delete(`/admin/products/${productId}/variants/${variantId}`),
  deleteVariantImage: async (productId: string, variantId: string, imageId: string) => api.delete(`/admin/products/${productId}/variants/${variantId}/images/${imageId}`),
  setPrimaryVariantImage: async (productId: string, variantId: string, imageId: string) => api.patch(`/admin/products/${productId}/variants/${variantId}/images/${imageId}/primary`),
  scanProductByBarcode: async (barcode: string) => api.get(`/admin/products/scan/${encodeURIComponent(barcode)}`),
  getProductsByCategory: async (categoryId: string) => api.get(`/admin/products?category=${categoryId}&limit=100`),
